...
2) Установить Java версии не ниже 8. Создать системную переменную JAVA_HOME и указать путь к JDK(например C:\Program Files\Java\jdk1.8.0_251).
Также указать путь к JDK в переменной PATH.
3) Установить Maven версии не ниже 3.6.1. Скачать архив с сайта https://maven.apache.org/download.cgi.
Следуя указаниям инструкции из файла README, установить maven; указать путь к maven в переменной PATH.
4) Проверить установленные Java и Maven: в командной строке ввести команды "java -version" и "mvn --version".
3) Установить IntelliJ IDEA с сайта jetbrains.com/idea/ (если необходимо).
4) Открыть командную строку в корневой директории проекта(Удерживая Shift, нажать правой кнопкой мыши на корневой каталог проекта и выбрать Открыть PowerShell здесь).
Ввести команды 
mvn clean
mvn compile
5) Простой запуск демо:
mvn exec:java -Dexec".mainClass"="com.andrewn.Application"

Запуск тестов:
mvn test

Запуск теста производительности JMH осуществлять из IDEA.
...
Для каждой итерации тестирования производительности определено одно и то же количество операций, которое
